...
export function useImageProcessor() {
  ...
  const processImage = async (image, boxes, displaySize = null, config = {}) => {
    ...
    try {
      ...
      // 计算预览→原始缩放比例，用于正确缩放 borderWidth / borderRadius
      const previewToOriginalScale = {
        x: originalImg.width / globalCoordinateMapper.previewSize.width,
        y: originalImg.height / globalCoordinateMapper.previewSize.height
      }
      
      // 转换边框坐标（从原始坐标到导出坐标，并应用缩放）
      const originalBoxes = boxes.map(box => {
        // x/y/w/h 已经是原始图片坐标，直接应用导出缩放
        // borderWidth/borderRadius 是预览逻辑像素，需要先缩放到原始图片比例再应用导出缩放
        const scaledBox = {
          ...box,
          x: box.x * scale,
          y: box.y * scale,
          ...
          borderWidth: box.borderWidth * previewToOriginalScale.x * scale,
          borderRadius: box.borderRadius * previewToOriginalScale.x * scale
        }
   ...
   */
  const canvasToBlob = (canvas, format = 'png', quality = 0.9, timeoutMs = 30000) => {
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('导出超时，请降低分辨率后重试')), timeoutMs)
      try {
  ...
  const validateBoxes = (boxes, image) => {
    ...
    return boxes.map(box => {
      const constrained = globalCoordinateMapper.constrainCoordinates(
        ...
        box.width, 
        box.height, 
        false // 原始坐标 — 存储中的 x/y/w/h 是原始图片坐标
      )
